...
-- プロジェクト更新時のトリガー
DROP TRIGGER IF EXISTS update_project_metadata_trigger ON projects;
...
-- リアルタイム更新のためのPublication作成（既存の場合は無視）
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_publication WHERE pubname = 'project_changes') THEN
